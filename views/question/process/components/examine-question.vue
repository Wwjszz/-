<template>
  <a-card class="general-card">
    <h1 class="logo">{{ $t('menu.question.process') }}</h1>
    <a-row>
...
<script lang="ts" setup>
  ...
  const { t } = useI18n();
  const renderData = ref<QuestionNotExamineRecord[]>([]);
  ...
  const fetchData = async (
    ...
    try {
      ...
      renderData.value = data.list as QuestionNotExamineRecord[];
